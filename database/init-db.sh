...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" <<-EOSQL
    ...
    CREATE USER $DB_DB_USER WITH ENCRYPTED PASSWORD '$DB_DB_PASS';
    GRANT ALL PRIVILEGES ON DATABASE magicdocs TO $DB_DB_USER;
EOSQL

psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" -d magicdocs <<-EOSQL
    GRANT ALL ON SCHEMA public TO $DB_DB_USER;
EOSQL